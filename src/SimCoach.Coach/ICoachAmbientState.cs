...
/// has-reference) and the latest gate snapshot. Implemented by <see cref="LiveCoachAmbientState"/>, which backs
/// it with the sim-adapter car class + the reference lookup + a gate-only <c>TelemetryFanOut</c> latest-frame
/// subscription. (Lap number is tracked by <c>CoachService</c> off the event stream, not here.)
/// </summary>
