...
- **OD12 → DRY-GATE THE EMBEDDED LINE (new, owner, 2026-07-17).** The vendored embedded `alien_line` is keyed by track only and baked from a DRY hotlap. It activates as the LINE reference ONLY in dry conditions — the dry buckets share it (owner: "горячо+сухо и тепло+сухо разницы мало"), so `dry-cool ≈ dry-warm`. In `damp`/`wet` (`WeatherBucket` not `dry-*`) it is gated OFF and the session falls back to the centerline/PB line + the M7 present-but-inactive diagnostic (the pre-OD12 behavior). A triple-EXACT DB import (OD6) is unaffected — a genuine wet import still resolves. Gate lives in `ComputeSession` (sim-agnostic; keyed on the `dry-` bucket-name contract, not on `Adapters.ACC`). Boundary note: `damp` is treated as not-dry (conservative — no dry line on a slick drying surface).
