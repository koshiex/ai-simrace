...
>
> ### Скоуп P3 (решение владельца, 2026-07-05)
> - **Берём сейчас: M33–M39, M41, M42, M43** (всё кроме M40).
> - **M40 (streaming debrief) → Phase 4 (Voice/TTS):** чисто TTS-prep, «no P3 win» — перенесён в `implementation-plan.md` (Phase 4). Координировать с Phase-6 debrief-delivery + `StreamAsync`.
> - **Proto-политика:** P3 официально = contract-change фаза. Proto-добавления разрешены по мере нужды; каждое поле фиксируется (ADR/FR) и итоговый список полей владелец видит в плане ДО имплементации.
>
...
| M46 | **Теоретический идеал (эталон быстрее своего PB)**: per-corner best-ever по (track,car,weather,corner_id), персист между сессиями → «теор. лучший круг» = сумма лучших поворотов. Каждый поворот бьётся по отдельности, хотя целый PB — тяжело. Закрывает остаток M38 (self-median ≠ идеал). Решения: time-only vs channels, дебриф vs live vs оба, стор/миграция, драйвит ли live-коучинг. **NB: физический синтетический идеал из СОБСТВЕННЫХ данных (option A) фальсифицирован ДВАЖДЫ** — наивно и на измеренном g (см. `ideal-line-reference-research.md` §6: envelope-independence, grip-import под шумом). Абсолютный beyond-PB apex-speed из своих данных недостижим by construction → нужен ВНЕШНИЙ якорь (racing line из границ трассы / реплей-эталон через `AllowReplayCapture`). Безопасный остаток: ОТНОСИТЕЛЬНЫЙ коучинг (brake-release/throttle-timing vs PB). | reference-quality / product | large | `ReferenceStore`; `ComputeSession` (`_bestSectorMs` прецедент); `SessionEvent`/`GoldSessionPayload`/`DebriefTemplate` | in-game 2026-07-10 (свой PB тяжело бить); option A 2× falsified 2026-07-11 |
