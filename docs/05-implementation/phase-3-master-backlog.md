...
>
> ### Находки из теста в игре (билд #26, 2026-07-04)
> - **BUG M1-track-limits (высокий, user-visible):** coachable-гейт `IsValidLap && !IsInPitLane` + poison-латч глушат **весь остаток круга** при первом же не-coachable кадре. Срезка трек-лимитов на одном повороте (`IsValidLap→false` до конца круга) → 0 corner-советов на всём круге (подтверждено: сессия `20260704-100134-252`, невалидный круг получил только `lap_dirty`, ни одного corner-совета). Фикс: расцепить гейты — **эмиссия** живых советов по `!IsInPitLane` (+круг начат), а `IsValidLap` оставить только для **аккумуляции агрегатов** и сида эталона (out/in-круги всё равно режутся пит-кадрами; срезанный поворот глушит M3). Смотреть `CoachableFramePredicate.cs`, `ComputeSession.cs:107-119,239-269`.
> - **M44 — оверлей вместо тишины (идея владельца):** советы, срезанные лимитом каденса (M10) или иным гейтом, вместо полного дропа показывать в **оверлее** (визуально, без TTS) — инфо не теряется, аудио не засоряется. Зависит от построения оверлея (сейчас stub, ближе к P4); решение о маршрутизации `Silent(из-за каденса) → overlay` принять сейчас, реализовать когда оверлей появится. Смотреть `RuleEngine`/`CoachService` (точка `Silent`) + `SimCoach.Overlay`.
