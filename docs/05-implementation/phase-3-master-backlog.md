...
| M46 | **Теоретический идеал (эталон быстрее своего PB)**: per-corner best-ever по (track,car,weather,corner_id), персист между сессиями → «теор. лучший круг» = сумма лучших поворотов. Каждый поворот бьётся по отдельности, хотя целый PB — тяжело. Закрывает остаток M38 (self-median ≠ идеал). Решения: time-only vs channels, дебриф vs live vs оба, стор/миграция, драйвит ли live-коучинг. | reference-quality / product | large | `ReferenceStore`; `ComputeSession` (`_bestSectorMs` прецедент); `SessionEvent`/`GoldSessionPayload`/`DebriefTemplate` | in-game 2026-07-10 (свой PB тяжело бить) |
