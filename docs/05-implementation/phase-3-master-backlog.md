...
> - **M44 — оверлей вместо тишины (идея владельца):** советы, срезанные лимитом каденса (M10) или иным гейтом, вместо полного дропа показывать в **оверлее** (визуально, без TTS) — инфо не теряется, аудио не засоряется. Решение о маршрутизации `Silent(из-за каденса) → overlay` ратифицировано сейчас; реализация — в **Phase 5 (Overlay)**, записано в `implementation-plan.md`. Смотреть `RuleEngine`/`CoachService` (точка `Silent`) + `SimCoach.Overlay`.
