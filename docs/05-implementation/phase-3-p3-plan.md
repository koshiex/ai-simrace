...
> 7. **Два PR** (ратифицировано): PR-A «Правда+эталон» (M42,M43,M37,M34,M38) → мёрдж → PR-B «Расширение
>    коучинга» (M33,M35,M36,M41,M39). Строка «M34+M38 одним PR» снята. См. секцию «Секвенирование и батчинг».
>
...
## Прогресс имплементации (ветка `feat/phase-3-p3`, обновляется по мере коммитов)

**PR-A «Правда + эталон»** — в работе. Каждая задача = один коммит (build+test+format зелёные).
...
M34-populate (сертифицированная метрика не сдвинулась: «3929мс»/«+14799мс» мертвы). Владелец предоставил
доступ к `%LOCALAPPDATA%` — precondition закрывается без ручного прогона.

**Далее:** M34-coach → M38, затем весь дифф PR-A через независимый Strict→Defender→Judge, затем PR.
**PR-B** (M33, M35, M36, M41, M39) — после мёрджа PR-A.
...
**Батчинг (ратифицировано владельцем 2026-07-05): ДВА PR, последовательно.**

- **PR-A «Правда + эталон»** — M42, M43, M37, M34, M38. Спайн правдивости/референса; ground-truth
  merge-precondition (`M34-populate`, `M38-linedev`) целиком внутри этого PR. Внутренний порядок коммитов —
  как в таблице (M42 → M43 → M37 → M34 → M38), ADR-коммит открывает свой блок. В игре тестируется ядро P3:
  абсолютный эталон коучит медленного-но-ровного пилота (self-PB-дельты ≈ 0).
- **PR-B «Расширение коучинга»** — M33, M35, M36, M41, M39. Новые детекторы + богаче дебриф. Открывается
  **после мёрджа PR-A** (off обновлённого main), чтобы правки `CornerEvent`/`AggregatedLoss` не пушились из
  двух открытых PR разом и фундамент устаканился первым. В игре — новые подсказки + пост-сессия.

Оба PR заведомо превышают ~600-строчный потолок ревью — осознанный размен на меньшее число PR (владелец);
компенсируется тем, что каждый PR связный и in-game-тестируемый по отдельности, а diff-ревью идёт S→D→J
покомпонентно перед PR. Внутри PR: контрактный коммит — первым в своём M-блоке; M36 — поверх M35.
