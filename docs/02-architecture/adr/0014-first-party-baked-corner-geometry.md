...
2. **Differentiate once, detect on sign-stable channels.** Compute heading `θ = atan2(Δz, Δx)` and signed
   ...
   **total-variation-of-heading** `∫|dθ|` (always positive) — both sign-insensitive — thresholded at
   `|G| ≥ 1.0 g OR R < 180 m`. Split close complexes at a curvature **sign change** (±0.0015 rad/m) co-located
   with a `|G|` valley (0.65× of the flanking peaks). Apply a **post-split** `min-arc ≥ 35 m` + min-load
   filter (the splitter over-fragments otherwise). **Apex = argmax `|κ|`** of the centerline, never argmax of
   `G` (flat/fast corners have a plateaued, displaced `G` peak). Window = brake-onset → apex → throttle-resume.
